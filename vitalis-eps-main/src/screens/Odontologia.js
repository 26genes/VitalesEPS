...
const ODONTOLOGOS = [
  'Dra. María Fernández - Ortodoncia',
  'Dr. Carlos Gómez - Cirugía Oral',
  'Dra. Laura Vargas - Periodoncia',
  'Dr. Javier Ramírez - Endodoncia'
];

export default function AgendaCitaOdontologia({ navigation }) {
  ...
  const validarFormulario = () => {
    const { dia, hora, nombre, apellido, cedula, correo, odontologo, motivo } = form;
    const errores = [];
    ...
    if (!odontologo) errores.push('Odontólogo es requerido');
  ...
  const handleSubmit = async () => {
    ...
    if (success) {
      Alert.alert(
        'Cita Confirmada', 
        `Tu cita odontológica ha sido agendada para el ${form.dia} a las ${form.hora} con ${form.odontologo.split(' - ')[0]}`,
        [
          { 
            text: 'OK', 
            onPress: () => {
    ...
    Alert.alert(
      ...
      [
        ...ODONTOLOGOS.map(odontologo => ({
          text: odontologo,
          onPress: () => handleChange('odontologo', odontologo)
        })),
  ...
  const exportarCitas = async () => {
    try {
      const citas = await getCitas();
      const citasOdontologia = citas.filter(c => c.tipoCita === 'Odontología');

      if (!citasOdontologia.length) {
        Alert.alert('No hay citas', 'No hay citas odontológicas para exportar.');
        return;
      }

      const contenido = citasOdontologia.map(cita => 
        `Paciente: ${cita.nombre} ${cita.apellido}
Cédula: ${cita.cedula}
Fecha: ${cita.dia} ${cita.hora}
Odontólogo: ${cita.odontologo}
Motivo: ${cita.motivo}
Alergias: ${cita.alergias || 'Ninguna'}
Última visita: ${cita.ultimaVisita || 'No registrada'}
Visitado antes: ${cita.visitado ? 'Sí' : 'No'}
Correo: ${cita.correo}
Fecha registro: ${new Date(cita.fechaCreacion).toLocaleString()}\n`
      ).join('\n');

      const fileUri = FileSystem.documentDirectory + 'citas_odontologia.txt';
      await FileSystem.writeAsStringAsync(fileUri, contenido);

      await Sharing.shareAsync(fileUri, {
        mimeType: 'text/plain',
        dialogTitle: 'Exportar citas odontológicas',
...
Odontólogo: ${form.odontologo || 'No seleccionado'}
Motivo: ${form.motivo || 'No especificado'}
Alergias: ${form.alergias || 'Ninguna registrada'}
Última visita: ${form.ultimaVisita || 'No registrada'}
      ...
      <View style={styles.container}>
        ...
        <View style={styles.formContainer}>
          ...
          <TextInput
            ...
            style={[styles.input, activeField === 'alergias' && styles.activeInput]}
            value={form.alergias}
          ...
          <TextInput
            placeholder="Última visita al odontólogo (opcional)"
            style={[styles.input, activeField === 'ultimaVisita' && styles.activeInput]}
            value={form.ultimaVisita}
            onChangeText={(text) => handleChange('ultimaVisita', text)}
            onFocus={() => handleFocus('ultimaVisita')}
            onBlur={handleBlur}
          />
          ...
          <TouchableOpacity 
            ...
            onPress={seleccionarOdontologo}
          >
            <MaterialIcons name="medical-services" size={18} color="#0E8CA4" />
            <Text style={[styles.medicoText, !form.odontologo && styles.placeholderText]}>
              {form.odontologo || 'Seleccionar odontólogo'}
            </Text>
